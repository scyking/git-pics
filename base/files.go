...
func init() {
	...
	fileTypeMap.Store("6D6F6F76", "mov")         //Quick time (mov)
...
func isImageFile(filepath string) (bool, error) {
	...
	ext := GetFileType(src[:10])

	for _, e := range []string{"jpg", "png", "gif", "bmp", "tif"} {
		if e == ext {
			return true, nil
		}
	}
...
func ImageFileNames(filePath string) ([]string, error) {
	...
	for _, file := range files {
		...
		if ok, err := isImageFile(filepath.Join(filePath, file.Name())); ok {
